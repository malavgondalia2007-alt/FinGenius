...
export function Onboarding() {
  ...
  const shouldShowTypeStep = profileData.age !== undefined && profileData.age >= 18;
  return <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-12">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          {step === 2 && <AgeStep onNext={(age) => handleNext({
          age
        })} onBack={handleBack} />}

          {/* Type selection step - only shown for users 18+ */}
          {step === 3 && shouldShowTypeStep && <TypeStep onNext={(type) => handleNext({
          type
        })} onBack={handleBack} />}

          {/* Financial data step - step 3 for under 18, step 4 for 18+ */}
          {(step === 3 && !shouldShowTypeStep || step === 4 && shouldShowTypeStep) && <>
              {profileData.type === 'student' && <StudentDataStep onNext={(data) => {
            setProfileData((prev) => ({
              ...prev,
              ...data
            }));
            setStep(step + 1);
              ...
              {profileData.type === 'employee' && <EmployeeDataStep onNext={(data) => {
            setProfileData((prev) => ({
              ...prev,
              ...data
            }));
          ...
          {(step === 4 && !shouldShowTypeStep || step === 5 && shouldShowTypeStep) && <CompletionStep onFinish={handleFinish} />}
        </div>
      </main>
    </div>;
}