...
import { GoalAvailableBalance } from '../components/GoalAvailableBalance';
import { GoalFeasibilityChecker } from '../components/GoalFeasibilityChecker';
export function Goals() {
  ...
  // Calculate Net Savings for Feasibility
  const { netSavings } = profile ?
    calculateNetSavings(profile, expenses, income) :
    {
      netSavings: 0
    };
  // Calculate Available Balance
  const { availableForNewGoals } = profile ?
  ...
  const handleAddFunds = (id: string, amount: number) => {
    ...
    const validation = validateGoalContribution(amount, availableForNewGoals);
  ...
  return (
    <>
      {/* Error Toast */}
      {errorMessage &&
        <div className="fixed top-24 right-8 bg-red-500 text-white px-6 py-4 rounded-xl shadow-lg animate-fade-in z-50 flex items-start gap-3 max-w-md">
          <AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-bold text-sm">Contribution Blocked</p>
