...
export function AdminExpenses() {
  ...
  const columns = [{
    ...
    key: 'type',
    label: 'Type',
    render: (value: string) => <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${value === 'essential' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
          {value === 'essential' ? 'Essential' : 'Non-Essential'}
        </span>
  }, {
    key: 'amount',
    label: 'Amount',
    render: (value: number) => <span className="font-semibold text-slate-900">
  ...
  return <AdminLayout>
      <div className="space-y-6">
        ...
        {/* Table */}
        <AdminTable columns={columns} data={expensesWithUserNames} emptyMessage="No expenses recorded yet" />
      </div>
    </AdminLayout>;
}