...
import { Server, Database, Shield, Activity, Clock, HardDrive } from 'lucide-react';
  ...
  return <AdminLayout>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, idx) => <div key={idx} className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`w-3 h-3 rounded-full ${service.status === 'Operational' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`font-mono font-medium ${service.latency !== 'N/A' && parseInt(service.latency) > 500 ? 'text-amber-600' : 'text-slate-900'}`}>
                    {service.latency}
                  </span>
                </div>
              </div>
