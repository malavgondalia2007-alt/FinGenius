...
import { Target, TrendingDown, AlertCircle, Settings, BrainCircuit } from 'lucide-react';
...
export function AdminGoalsIntelligence() {
  ...
  const abandonmentRate = totalGoals > 0 ? zeroContributionGoals.length / totalGoals * 100 : 0;
  // Category Breakdown
  const categories = goals.reduce((acc, goal) => {
    acc[goal.category] = (acc[goal.category] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
  const sortedCategories = Object.entries(categories).sort(([, a], [, b]) => b - a).slice(0, 5);
  return <AdminLayout>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <AdminMetricCard title="Total Active Goals" value={totalGoals} icon={Target} iconBgColor="bg-blue-100" />
          <AdminMetricCard title="Abandonment Rate" value={`${abandonmentRate.toFixed(1)}%`} icon={TrendingDown} iconBgColor="bg-red-100" trend={{
          value: 2.5,
          label: 'increase'
        }} />
          <AdminMetricCard title="Zero-Contribution Goals" value={zeroContributionGoals.length} icon={AlertCircle} iconBgColor="bg-amber-100" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
              {zeroContributionGoals.length === 0 ? <p className="text-slate-500 text-center py-8">
                  No at-risk goals detected.
                </p> : zeroContributionGoals.map((goal, idx) => <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <div>
