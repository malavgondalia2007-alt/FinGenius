...
import {
  Expense,
  Income,
  SmartWarning as SmartWarningType,
  Goal } from
'../types';
import {
...
import {
  calculateNetSavings,
  calculateRealisticTimeline } from
...
export function Expenses() {
  ...
  const getGoalImpact = () => {
    if (!profile || goals.length === 0 || expenses.length === 0) return null;
    // Find top non-essential category
    const nonEssentialExpenses = expenses.filter(
      (e) => e.type === 'non-essential'
    );
    const categoryTotals = nonEssentialExpenses.reduce(
      (acc, e) => {
    ...
    // Calculate impact
    const { netSavings } = calculateNetSavings(profile, expenses, incomes);
    const potentialSavings = amount * 0.5; // Assume 50% reduction
    const newNetSavings = netSavings + potentialSavings;
    const currentTimeline = calculateRealisticTimeline(
      nearestGoal.targetAmount,
      nearestGoal.savedAmount,
      netSavings
    );
    const improvedTimeline = calculateRealisticTimeline(
      nearestGoal.targetAmount,
      nearestGoal.savedAmount,
      newNetSavings
    );
    const monthsSaved =
    currentTimeline.realisticMonths - improvedTimeline.realisticMonths;
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-purple-800">
                  Reducing your{' '}
                  <span className="font-bold">{impact.categoryName}</span>{' '}
                  spending by
                  <span className="font-bold">
                    {' '}
                    ₹{impact.potentialSavings.toLocaleString()}
                  </span>{' '}
                  could help you reach your
                  <span className="font-bold"> {impact.goalName}</span> goal
                  <span className="font-bold text-emerald-600">
                    {' '}
                    {impact.monthsSaved} months faster!
                  </span>{' '}
                  🚀
                </p>
              </div>
