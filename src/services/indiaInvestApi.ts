...
/**
 * Get recommended funds based on risk profile for SIP calculations
 */
export async function getSIPFundRecommendations(
riskProfile: 'low' | 'medium' | 'high')
: Promise<any[]> {
  // Simulate API delay
  await new Promise((resolve) => setTimeout(resolve, 800));

  // In a real app, this would fetch from the live API endpoint
  // Returning mock data based on real market trends for demonstration
  const mockFunds = [
  // Low Risk (Debt/Liquid)
  {
    id: 'debt-1',
  ...
  return mockFunds.filter((f) => f.riskLevel === riskProfile);
}