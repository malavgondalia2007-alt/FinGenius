...
import { Home, Utensils, Car, GraduationCap, CreditCard, Zap, Smartphone, Film, Heart, MoreHorizontal, EyeOff, AlertCircle, Plus } from 'lucide-react';
...
export function CommitmentCategoryEditor({
  ...
  const handleAmountChange = (id: string, amount: string) => {
    ...
    const newCategories = categories.map((c) => c.id === id ? {
      ...c,
      amount: numAmount
    } : c);
  ...
  const getStatusBorder = (category: CommitmentCategory) => {
    ...
    if (category.amount > category.recommendedMax) return 'border-red-200 bg-red-50';
    if (category.amount > category.recommendedMin) return 'border-amber-200 bg-amber-50';
    return 'border-emerald-200 bg-emerald-50';
  };
  const totalCommitments = categories.filter((c) => !c.hidden).reduce((sum, c) => sum + c.amount, 0);
  ...
  return <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            scale: 0.95
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg bg-white ${getStatusColor(category)}`}>
                  ...
                  <button onClick={() => toggleHidden(category.id)} className="text-gray-400 hover:text-gray-600 transition-colors" title="Hide category">
                ...
                <div className="relative">
                  ...
                  <input type="number" value={category.amount || ''} onChange={(e) => handleAmountChange(category.id, e.target.value)} className="w-full pl-7 pr-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="0" />
                </div>

                {isHigh && <div className="flex items-center gap-1 mt-2 text-xs text-red-600">
        ...
        <div className="col-span-1 md:col-span-2 flex flex-wrap gap-2 mt-4">
          {categories.filter((c) => c.hidden).map((category) => <button key={category.id} onClick={() => toggleHidden(category.id)} className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-dashed border-gray-300 text-gray-500 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50 transition-all text-sm">
      ...
      <div className="sticky bottom-0 bg-white/90 backdrop-blur-sm p-4 rounded-xl border border-gray-200 shadow-lg">
        ...
        <div className="w-full bg-gray-100 h-2 rounded-full mt-3 overflow-hidden">
          <div className={`h-full transition-all duration-500 ${remaining > 0 ? 'bg-emerald-500' : 'bg-red-500'}`} style={{
