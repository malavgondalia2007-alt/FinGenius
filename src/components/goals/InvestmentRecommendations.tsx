...
import { TrendingUp, Shield, Info, ArrowRight, AlertTriangle } from 'lucide-react';
...
export function InvestmentRecommendations({
  ...
  return <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        {recommendations.map((rec, index) => <motion.div key={rec.fundId} initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        delay: index * 0.1
            ...
            <Card className="p-4 hover:shadow-md transition-shadow border-l-4 border-l-indigo-500">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium flex items-center gap-1 ${getRiskColor(rec.risk)}`}>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                ...
                <Button variant="outline" className="text-xs h-8 px-3" onClick={() => onSelect?.(rec.fundId)}>
