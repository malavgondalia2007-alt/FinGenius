...
export function StudentDataStep({
  ...
  return <div className="max-w-md mx-auto animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Input type="number" label="Weekly Pocket Money (₹)" placeholder="e.g. 2000" value={pocketMoney} onChange={(e) => setPocketMoney(e.target.value)} required />

        <Input type="number" label="Estimated Weekly Expenses (₹)" placeholder="e.g. 1500" value={expenses} onChange={(e) => setExpenses(e.target.value)} required />
        ...
        <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-800">
          <p>
            ...
            <span className="font-bold">
              ₹
              {Math.max(0, (parseFloat(pocketMoney) || 0) - (parseFloat(expenses) || 0))}
            </span>
          </p>
        </div>
